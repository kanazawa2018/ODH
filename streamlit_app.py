...
# === 追加: アバター解決ユーティリティ関数 ===
def resolve_assistant_avatar(user_animal: str, char_df: pd.DataFrame | None):
    """
    画像の探索規則:
      1) キャラクター名.{jpg,jpeg,png,webp}
      2) 動物名.{jpg,jpeg,png,webp}
    最初に見つかったパスを返す。見つからなければ (None, 表示名) を返す。
    表示名は「キャラクター名が取れればそれを、なければ動物名」。
    """
    display_name = user_animal
    candidate_basenames = []

    if char_df is not None:
        row = char_df[char_df['動物'] == user_animal]
        if not row.empty and 'キャラクター名' in row.columns:
            display_name = str(row['キャラクター名'].iloc[0])
            candidate_basenames.append(display_name)

    # フォールバック: 動物名でも探索
    candidate_basenames.append(user_animal)

    exts = [".jpg", ".jpeg", ".png", ".webp"]
    for base in candidate_basenames:
        for ext in exts:
            p = os.path.join(CHAR_IMAGE_DIR, f"{base}{ext}")
            if os.path.exists(p):
                return p, display_name

    # 何も見つからない場合はパスなし（初期表示で警告は出さない）
...
with tab3:
    ...
    if len(users_df) > 0:
        ...
        if user_name:
            ...
            # ← 修正点：アバターと表示名を関数で解決（初期表示で警告は出さない）
            assistant_avatar_path, assistant_char_name = resolve_assistant_avatar(user_animal, char_df)
